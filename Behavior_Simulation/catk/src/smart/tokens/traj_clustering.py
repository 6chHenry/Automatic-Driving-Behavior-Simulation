...
'''
    从原始agent轨迹提取典型的动作集ai形成vocabulary
'''
import pickle
...
def Kdisk_cluster(
    X,  # [n_trajs, 4, 2], bbox of the last point of the segment 轨迹上最后一点的四角坐标
    N,  # int    词汇表大小
    tol,  # float   tolerance
    a_pos,  # [n_trajs, 6, 3], the complete segment完整轨迹   6是6个时间步 3是(x,y,朝向角) x,y是指的位移
    ...
    for i in range(N):
        if i == 0:
            ...
            choice_index = torch.randint(0, X.shape[0], (1,)).item()   # 随机选择一个点为中心
        x0 = X[choice_index]

        # res_mask = torch.sum((X - x0) ** 2, dim=[1, 2]) / 4.0 > (tol**2)
        res_mask = torch.norm(X - x0, dim=-1).mean(-1) > tol   # 得到一个[n]向量,大于tol位置为True
        if cal_mean_heading:
...
if __name__ == "__main__":
    ...
    for k, v in data.items():
        ...
        )  # [n_trajs, 4, 2]   计算轨迹终点边界框（矩形轮廓）仍然是在相对坐标系下的
        ...
        ret_traj[:, :, -1] = wrap_angle(ret_traj[:, :, -1])   # 对朝向角规范化   [N, 6, 3]
        ...
        res["token_all"][k] = contour.numpy()   #  [N, 6, 4, 2]    ???
