...
def run(cfg: DictConfig) -> None:    
    if cfg.get("seed"):    
        L.seed_everything(cfg.seed, workers=True)   # keep the same random seed
    ...
    # setup model watching    NET WORK ERROR
    for _logger in logger:
        if isinstance(_logger, WandbLogger):
            _logger.watch(model, log="all")    # trace parameters and gradients
